...
galaxytoolbar.GTPlugin_messages={
	...
	submit_player_messages_and_activities: function(doc, dom_content) {
		try {
			...
			var oChatBarPlayerList = dom_content.getElementById("chatBarPlayerList"); // directly loaded within each page and not in the messages section
			if (!oChatBarPlayerList) {
				oChatBarPlayerList = dom_content.getElementsByClassName("playerlist")[0]; // only valid for the messages section
			}
	...
	submit_v6_combat_report(doc, message_id, ajax_content) {
		try {
			...
			*/
			...
			// Firefox fallback as ajax_content returned always "not well formed" quite often and the DOM should is refreshed by now
			...
			dom_content.documentElement.innerHTML = ajax_content;
		...
		try {
			...
			for (var i = 0; i < content.length; i++) {
				try {
					...
					var time = parseInt(Date.UTC(my_date.getUTCFullYear(),my_date.getUTCMonth(),
										my_date.getUTCDate(),my_date.getUTCHours(),my_date.getUTCMinutes(),
										my_date.getUTCSeconds())/1000)+minute*60+second;
					minute = 0;
					second = 0;
