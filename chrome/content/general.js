...
galaxytoolbar.GTPlugin_general = {
	...
	selectedRanks: -1,
	debug_mode: false,
	is_summertime: new Object(),
	//always use format "major.minor.revision"
	version: "2.8.4",
	tested_with_ogame_version : "6.1.7",
	
	getLocString: function(string) {
		try {
			var locString = document.getElementById("galaxytoolbarStrings").getString("gt_"+string);
		...
		try {
			...
			switch (status) {
				case 0:
					...
					break;
				case 1:
					picture.setAttribute("src","chrome://galaxyplugin/skin/images/green.png");
					break;
				case 2:
					picture.setAttribute("src","chrome://galaxyplugin/skin/images/yellow.png");
					break;
				case 3:
					picture.setAttribute("src","chrome://galaxyplugin/skin/images/red.png");
					break;
			}
			...
			if (Data) {
				...
				if (MenuButton && ! versionCheck) {
					...
					try {
						...
						// from Mozilla 34 onwards, a window object shall be given as first argument
                        // of createStorage
                        // see: https://bugzilla.mozilla.org/show_bug.cgi?id=660237#c43
                        // and https://addons.mozilla.org/de/developers/addon/galaxytoolbar/validation-result/350430
			            var bIsOldMoz = Services.vc.compare(Services.appinfo.platformVersion, "34.0a") < 0;
			            storage = bIsOldMoz ? Services.domStorageManager.createStorage(principal, "")
			                                : Services.domStorageManager.createStorage(null, principal, "");
					
					} catch(e) {
						// fallback for Firefox < 23
						dsm = Components.classes["@mozilla.org/dom/storagemanager;1"]
				          .getService(Components.interfaces.nsIDOMStorageManager);
					...
					if (storage) {
						var value					= storage.getItem('OGameVersionCheck') || '';
					...
					if ( count < 11 && MenuButton.style.color != '#FF4B00') {
						if (storage)			{ storage.setItem('OGameVersionCheck', ogameVersion + '|' + (++count) ); }
	...
	update_check: function() {
		// check if update is needed - also run SQLite updates here!
		var old_version;
		try {
			old_version = galaxytoolbar.GTPlugin_general.Prefs.getCharPref("gtplugin.settings.version");
		} catch(pref_not_found) {
			//was not installed yet
			galaxytoolbar.GTPlugin_general.Prefs.setCharPref("gtplugin.settings.version", decodeURIComponent(this.version));
			return;
		}
		...
		if (this.version != old_version) {
			...
			// execute update
			if (!galaxytoolbar.GTPlugin_general.compare_versions(old_version[0],old_version[1],old_version[2],2,2,0)) {
				// set new pref. at 2.2 (new ogame version)
				galaxytoolbar.GTPlugin_general.Prefs.setCharPref("gtplugin.status_style_window", decodeURIComponent("position:fixed;top:10px;left:10px;z-index: 999;"));
				galaxytoolbar.GTPlugin_general.Prefs.setCharPref("gtplugin.status_style_header", decodeURIComponent("font-weight:bold;"));
				galaxytoolbar.GTPlugin_general.Prefs.setCharPref("gtplugin.status_style_results", decodeURIComponent("color:white;"));
			}
			
			if (!galaxytoolbar.GTPlugin_general.compare_versions(old_version[0],old_version[1],old_version[2],2,3,0)) {
				// copy pref settings to DB
				galaxytoolbar.GTPlugin_storage.copy_pref_data_to_db();
				galaxytoolbar.GTPlugin_general.Prefs.setCharPref("gtplugin.settings.version", decodeURIComponent(this.version));
				// the sqlite was created before, so no need to alter the tables once again (with update 2.4/2.5) -> return
				return;
			}
			
			if (!galaxytoolbar.GTPlugin_general.compare_versions(old_version[0],old_version[1],old_version[2],2,4,0)) {
				galaxytoolbar.GTPlugin_storage.install_update_2_4();
			}
			
			if (!galaxytoolbar.GTPlugin_general.compare_versions(old_version[0],old_version[1],old_version[2],2,5,0)) {
				galaxytoolbar.GTPlugin_storage.install_update_2_5();
			}
			
			if (!galaxytoolbar.GTPlugin_general.compare_versions(old_version[0],old_version[1],old_version[2],2,6,0)) {
				galaxytoolbar.GTPlugin_storage.install_update_2_6();
			}
			...
			if (!galaxytoolbar.GTPlugin_general.compare_versions(old_version[0],old_version[1],old_version[2],2,8,0)) {
				galaxytoolbar.GTPlugin_storage.install_update_2_8();
			}
	...
	get_ogame_time: function(doc) {
		try {
			var ogame_servertime = new Date();
			if (this.compare_ogame_version_with(doc,5,0,0)) {
				var startServerTime = parseInt(window.content.wrappedJSObject.startServerTime);
				
				if (isNaN(startServerTime)) return false;
				
				var time = parseInt(window.content.wrappedJSObject.timeDiff);
				
				if (isNaN(time)) return false;
		
				ogame_servertime.setTime(ogame_servertime.getTime() + time);
			} else {
				var startServerTime = parseInt(window.content.wrappedJSObject.startServerTime);
				
				if (isNaN(startServerTime)) return false;
				
				var script = this.get_script_of_page(doc);
				
				if (script == "") return false;
				
				script = script.substring(script.indexOf("currTime.setTime"),script.length);
				script = script.substring(0,script.indexOf(";"));
				var time = parseInt(script.match(/(\d+)/)[1]);
				
				ogame_servertime.setTime((time-startServerTime)+ ogame_servertime.getTime());
			}
			return ogame_servertime;
		} catch (e) {
			return false;
		}
	},
	...
	get_tzd: function(host) {
		return this.is_summertime[host];
	},
	...
	log_to_console: function(text) {
		try {
			...
			var My_ConsoleService =
			  Components.
			  classes['@mozilla.org/consoleservice;1'].
			  getService(Components.interfaces.nsIConsoleService);
			
			My_ConsoleService.logStringMessage('Galaxytool extension: ' + text);
	...
	// from here: https://developer.mozilla.org/en/Code_snippets/HTML_to_DOM#Safely_parsing_simple_HTML.c2.a0to_DOM
	HTMLParser : function(aHTMLString){
		...
		try {
			...
			body.appendChild(parserUtils.parseFragment(aHTMLString,parserUtils.SanitizerAllowStyle,false,null,body));
		} catch(e) {
